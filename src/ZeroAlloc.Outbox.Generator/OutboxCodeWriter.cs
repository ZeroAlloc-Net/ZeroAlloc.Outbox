...
internal static class OutboxCodeWriter
{
    ...
    // IOutboxSerializer's Serialize/Deserialize carry [RequiresUnreferencedCode]
    // and [RequiresDynamicCode]. A file-level `#pragma warning disable` is C#
    // compile-time only — it does NOT survive ILC's publish-time analysis. The
    // generator instead emits [UnconditionalSuppressMessage] attributes on the
    // specific methods that call the serializer, which ARE serialised into IL
    // metadata and honoured by ILC.
    ...
    private static void AppendWriter(StringBuilder sb, string typeFqn, string writerName, string typeKey)
    {
        ...
        sb.AppendLine("    }");
        sb.AppendLine();
        sb.AppendLine(TrimSuppress);
        sb.AppendLine(AotSuppress);
